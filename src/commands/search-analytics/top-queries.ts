...
export const topQueriesCommand: CommandDefinition = {
  name: 'top_queries',
  group: 'search-analytics',
  subcommand: 'top-queries',
  description: 'Convenience wrapper: top search queries by clicks for a date range (dimensions=query).',
  examples: ['gsc search-analytics top-queries --days 28 --row-limit 25', 'gsc search-analytics top-queries'],
  readOnly: true,
  ...
  handler: async (input, client) => {
    ...
    const requestBody = buildQueryBody({ ...input, dimensions: 'query', rowLimit: input.rowLimit ?? 25 });
